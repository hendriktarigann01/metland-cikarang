...
const TOTAL_FRAMES = HERO_CONTENT.totalFrames; // 240
const FRAME_DIR = HERO_CONTENT.frameDir; // /frame-metland-parallax
const FRAME_NAME = HERO_CONTENT.frameName; // frame_
...
export function HeroFrameBackground() {
  ...
  const loadedRef = useRef(0); // jumlah frame yang sudah ter-load
  const rafRef = useRef<number>(0);

  // Refs untuk Loading Progress Bar di bagian bawah (CreativeLabV2 Pattern)
  ...
  // Main lifecycle effect (runs on client only, after mount)
  useEffect(() => {
    const canvas = canvasRef.current;
    ...
    // ── Scroll → frame mapping (identik pola Envision) ───────────
    let targetFrame = 0;
    let currentFrame = 0;
    let opacity = 1;
    ...
    const preload = async () => {
      ...
      // Load sisa frame secara idle di background
      let i = PRELOAD_COUNT;
      const idleLoad = () => {
        if (i < TOTAL_FRAMES) {
          const batch = 5;
    ...
    <>
      ...
      >
        ...
        <div className="w-full h-1.5 bg-brand-primary/20 backdrop-blur-xs">
          <div
            ref={progressBarRef}
            className="h-full bg-gradient-to-r from-brand-primary to-brand-secondary shadow-[0_0_8px_rgba(16,153,141,0.5)] origin-left w-full"
